...
spark = SparkSession.builder.master("local").appName("AutoSalesProject").getOrCreate() 

#Read data.csv in a RDD . important to reference hdfs namenode file path for context
raw_rdd = spark.sparkContext.\
           textFile("hdfs://172.23.0.4:9000/sparkmini/data.csv")


# Lambda -Map on the original RDD to create new RDD of key,value pair i.e ((vin_number),make,year,incident_type)
...
# Lambda- GroupByKey is applied on above RDD of (K,V) as input to get an output RDD of (K,Iterable) pairs i.e. (vin_number, Iterable((make1,year1),(make2,year2)...)
...
# Lambda-Map to get a RDD of ((make, year),1)
...
# Save the final RDD 'accident_rec_per_year' in folder . saved to spark submit bin path
accident_rec_per_year_frmt.saveAsTextFile("accident_records_count-2021")